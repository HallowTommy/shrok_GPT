...
"""  # Оставил только начало промпта для читаемости
...
def generate_shrokai_response(user_input, history):
    logging.info("🤖 Генерация ответа для пользователя...")
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        send_to_tts(welcome_message)  # Отправляем в TTS, но не показываем ссылку в чате
        ...
        while True:
            ...
            send_to_tts(response)  # Отправляем в TTS, но НЕ показываем ссылку
